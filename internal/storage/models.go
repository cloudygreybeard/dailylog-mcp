...
type DailyLogEntry struct {
	...
	Type        string            `json:"type"` // "activity", "status", "note", "summary"
	Title       string            `json:"title"`
	Description string            `json:"description"`
	Tags        []string          `json:"tags,omitempty"`
	Status      int               `json:"status,omitempty"`   // 1-10 scale
...
type DayLog struct {
	...
	StatusAverage float64         `json:"status_average,omitempty"`
...
// calculateStatusAverage calculates the average status for the day
func (d *DayLog) calculateStatusAverage() {
	var total float64
	var count int

	for _, entry := range d.Entries {
		if entry.Status > 0 {
			total += float64(entry.Status)
			count++
		}
	}

	if count > 0 {
		d.StatusAverage = total / float64(count)
	} else {
		d.StatusAverage = 0
	}
}
